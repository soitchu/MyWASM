...
function _execFileSync(options, execOptions) {
  ...
  if (IS_WIN) {
    shellPath = globalThis.process.env.ComSpec || 'cmd.exe';
    globalThis.process.env.Q = '"'; // The quote (") that isn't escaped.
...
function readlineExt(options) {
  var res = {},
    execOptions = {env: globalThis.process.env, encoding: options.encoding},
    hostArgs, extMessage;

  if (!extHostPath) {
    if (IS_WIN) {
      if (globalThis.process.env.PSModulePath) { // Windows PowerShell
  ...
  if (IS_WIN && !globalThis.process.env.PSModulePath) { // Windows Script Host
...
function replaceHomePath(path, expand) {
  var homePath = pathUtil.normalize(
    IS_WIN ? (globalThis.process.env.HOMEDRIVE || '') + (globalThis.process.env.HOMEPATH || '') :
    globalThis.process.env.HOME || '').replace(/[/\\]+$/, '');
...
exports.promptSimShell = function(options) {
  ...
  return exports.prompt(margeOptions({
    ...
    prompt:             (function() {
      return IS_WIN ? '$<cwd>>' :
        // 'user@host:cwd$ '
        (globalThis.process.env.USER || '') +
        (globalThis.process.env.HOSTNAME ? '@' + globalThis.process.env.HOSTNAME.replace(/\..*$/, '') : '') +
        ':$<cwdHome>$ ';
    })()
  }));
  /* eslint-enable key-spacing */
